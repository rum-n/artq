...
const Feed = (props) => {
 ...
 const placeSubmitHandler = async event => {
  ...
  await sendRequest('https://artq-api-rum-n.vercel.app/api/saved','POST', formData)
} catch(err){}
};
  ...
  if (o.peoplewholiked.indexOf(auth.userId) > -1)
{
  ...
  await sendRequest(`https://artq-api-rum-n.vercel.app/api/images/likes/${o.id}/status/`,'PUT',JSON.stringify({
      "id" : o.id,
      "likes":updatedlikes,
    ...
    <div>
          <Col key={props.id} xs={3} md={4} className='art-cards'>
            <Card style={{ width: '18rem', marginBottom: '2rem', border: 'none'}} >
              <Card.Img src={`https://artq-api-rum-n.vercel.app/${props.url}`} /> 
            ...
            <Modal show={show} onHide={handleClose}>
                  ...
                  <Modal.Body>
                    <Container>
                      <Row>
                        <Col xs={12} md={6}>
                            <img className="modal-img" src={`https://artq-api-rum-n.vercel.app/${props.url}`} alt={activeItem.name}/>
                        </Col>
                        <Col xs={12} md={6}>
                          <h3>{props.title}</h3>
                          <p><b>Dimentions</b></p>
